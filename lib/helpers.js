 ...
 * @param {Array<Object>} events The collection of events.
 * @return {Boolean} Returns `true` if the event should be filtered, else `false`.
 */
function shouldFilterEvent(accumulator, event, events) {
  ...
  if (event.action === "created") {
    ...
    if (some(events, { action: "deleted", path: event.path })) {
      return false;
    }
  }
  ...
  if (event.action === "modified") {
    ...
    if (some(events, { action: "renamed", oldPath: event.path })) {
      return false;
    }
